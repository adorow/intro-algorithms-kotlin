...
interface SquareMatrixStrassen {
    companion object {

        fun from(m: Array<Array<Int>>): SquareMatrixStrassen =
                ArrayBasedSquareMatrixStrassen(m)
         ...
         */
        fun ofSize(n: Int, init: (Int, Int) -> Int = { _, _ -> 0 }): SquareMatrixStrassen =
                ArrayBasedSquareMatrixStrassen(
                        Array(n) { i ->
                            Array(n) { j ->
                                init(i, j)
                            }
    ...
    fun partition(i: Int, iLength: Int, j: Int, jLength: Int): SquareMatrixStrassen =
            SubSquareMatrixStrassen(this, i, iLength, j, jLength)

    operator fun plus(other: SquareMatrixStrassen) =
            SquareMatrixStrassen.ofSize(other.rows) { i, j ->
                this[i, j] + other[i, j]
            }

}

private class ArrayBasedSquareMatrixStrassen(private val m: Array<Array<Int>>) : SquareMatrixStrassen {
 ...
 */
private class SubSquareMatrixStrassen(
        private val delegate: SquareMatrixStrassen,
        private val iOffset: Int, private val iLength: Int,
        private val jOffset: Int, private val jLength: Int
) : SquareMatrixStrassen {
     ...
     */
    override fun partition(i: Int, iLength: Int, j: Int, jLength: Int): SquareMatrixStrassen {
...
/**
 * A SquareMatrixStrassen made up of joined partitions. The partitions are grouped as: top-left, top-right, bottom-left, bottom-right.
 * It is expected that all the partitions have the same dimensions.
 */
private class JoinedSubSquareMatrix(
        private val topLeft: SquareMatrixStrassen,
        private val topRight: SquareMatrixStrassen,
        private val bottomLeft: SquareMatrixStrassen,
        private val bottomRight: SquareMatrixStrassen
) : SquareMatrixStrassen {
    ...
    private fun getSection(i: Int, j: Int): SquareMatrixStrassen {
        checkIndex(i, j)
...
fun squareMatrixMultiplyRecursive(A: SquareMatrixStrassen, B: SquareMatrixStrassen): SquareMatrixStrassen {
    val n = A.rows

    if (n == 1) {
        return SquareMatrixStrassen.ofSize(n) { _, _ -> A[0, 0] * B[0, 0] }
    }
 ...
 */
fun squareMatrixMultiplyRecursive2(A: SquareMatrixStrassen, B: SquareMatrixStrassen): SquareMatrixStrassen {
    val n = A.rows

    val C = SquareMatrixStrassen.ofSize(n)
...
private fun squareMatrixMultiplyRecursive2_(A: SquareMatrixStrassen, B: SquareMatrixStrassen, C: SquareMatrixStrassen) {
    val n = A.rows
