...
When doing so, we can couch the solution with no cuts at all as saying that the first piece has size _i_ = _n_ and revenue _p<sub>n</sub>_ and that the remainder has size 0 with corresponding revenue _r<sub>0</sub>_ = 0.
...
We are given a sequence (chain)〈_A_<sub>1</sub>, _A_<sub>2</sub>, ..., _A_<sub>_n_</sub>〉of _n_ matrices to be multiplied, and we wish to compute the product
...
A product of matrices is **_fully parenthesized_** if it is either a single matrix or the product of two fully parenthesized matrix products, surrounded by parentheses.
For example, if the chain of matrices is 〈_A_<sub>1</sub>, _A_<sub>2</sub>, _A_<sub>3</sub>, _A_<sub>4</sub>〉, then we can fully parenthesize the product _A_<sub>1</sub>_A_<sub>2</sub>_A_<sub>3</sub>_A_<sub>4</sub> in five distinct ways:

(_A_<sub>1</sub>(_A_<sub>2</sub>(_A_<sub>3</sub>_A_<sub>4</sub>))),  
(_A_<sub>1</sub>((_A_<sub>2</sub>_A_<sub>3</sub>)_A_<sub>4</sub>)),  
((_A_<sub>1</sub>_A_<sub>2</sub>)(_A_<sub>3</sub>_A_<sub>4</sub>)),  
((_A_<sub>1</sub>(_A_<sub>2</sub>_A_<sub>3</sub>))_A_<sub>4</sub>),  
(((_A_<sub>1</sub>_A_<sub>2</sub>)_A_<sub>3</sub>)_A_<sub>4</sub>),  
...
The standard algorithm is given by the following pseudocode, which generalizes the Square-Matrix-Multiply from [Chapter 4][chapter4].
The attributes _rows_ and _columns_ are rows and columns in the matrix


**Pseudocode**:
>Matrix-Multiply(_A_, _B_)
&nbsp;&nbsp;&nbsp;&nbsp;    **if** _A_._columns_ ≠ _B_._rows_  
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;        **error** "incompatible dimensions"  
&nbsp;&nbsp;&nbsp;&nbsp;    **else** let _C_ be a new _A_._rows_ × _B_._columns_ matrix  
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;        **for** _i_ = 1 **to** _A_._rows_  
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;            **for** _j_ = 1 **to** _B_._columns_  
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;              _c_<sub>_ij_</sub> = 0  
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;              **for** _k_ = 1 **to** _A_._columns_  
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;              _c_<sub>_ij_</sub> = _c_<sub>_ij_</sub> + _a_<sub>_ik_</sub> * _b_<sub>_kj_</sub>  
&nbsp;&nbsp;&nbsp;&nbsp;    **return** _C_

**In Kotlin**: [Implementation](../src/main/kotlin/chapter15/MatrixMultiply.kt) and [Tests](../src/test/kotlin/chapter15/MatrixMultiplyTest.kt).
...
Biological applications often need to compare DNA of two or more different organisms.
...
We can represent those with the finite set {A, C, G, T}.
One reason to compare two strands of DNA is to define how "similar" they are, as a measure of how closely related the organisms are.
One way to define that similarity is to define that two strands of DNA are similar if one is a substring of the other.
...
Another way is to define a strand _S_<sub>3</sub>, such that its bases appear in each of _S_<sub>1</sub> and _S_<sub>2</sub>; these bases must appear in the same order, but not necessarily consecutively.
The longer the strand _S_<sub>3</sub> we can find, the more similar _S_<sub>1</sub> and _S_<sub>2</sub> are.

We formalise this notion of similarity as the longest-common-subsequence problem.

In the _**longest-common-subsequence problem**_, we are given two sequences _X_ = ⟨ _x_<sub>1</sub>, _x_<sub>2</sub>, ..., _x_<sub>m</sub> ⟩ and _Y_ = ⟨ _y_<sub>1</sub>, _y_<sub>2</sub>, ..., _y_<sub>n</sub> ⟩ and wish to find a maximum-length ommon subsequence of _X_ and _Y_.

**Pseudocode**:
>LCS-Length(_X_, _Y_)
TBD

>Print-LCS(_b_, _X_, _i_, _j_)
TBD
...
Suppose we are designing a program to translate text from English to French.
For each occurrence of an English word in the text, we need to look up its French equivalent.
We could perform these lookups by building a binary search tree with _n_ English words as key and the French translation as satellite data.
Because we'll search for each word, we want to spend as little time as possible with the searches.
We could ensure an _O_(lg _n_) search time per occurrence by using a red-black tree or any balanced binary search tree.
Words appear with different frequencies however, and common words such as _the_ may appear far from the root, while rarely used words would be closer to the root. 
Such an organisation would slow down the search.

What we need is known as an _**optimal binary search tree**_. 
Formally, we are given a sequence _K_ = ⟨ _k_<sub>1</sub>, _k_<sub>2</sub>, ..., _k_<sub>n</sub> ⟩ of _n_ distinct keys in sorted order, and we wish to build a binary search tree from these keys.
For each key _k_<sub>_i_</sub>, we have a probability _p_<sub>_i_</sub> that a search will be for _k_<sub>_i_</sub>.
Some searches may be for values not in _K_, and so we also have _n_ + 1 "dummy keys" representing values not in _K_.

For a given set of probabilities, we wish to construct a binary search tree whose expected search cost is smallest.
We call such a tree an _**optimal binary search tree**_.

>Optimal-BST(_p_, _q_, _n_)  
&nbsp;&nbsp;&nbsp;&nbsp;    let _e_\[1.._n_ + 1, 0.._n_], _w_\[1.._n_ + 1, 0.._n_]  
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;        and _root_\[1.._n_, 1.._n_] be new tables  
&nbsp;&nbsp;&nbsp;&nbsp;    **for** _i_ = 1 **to** _n_ + 1  
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;        _e_\[_i_, _i_ - 1] = _q_<sub>_i_-1</sub>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;        _w_\[_i_, _i_ - 1] = _q_<sub>_i_-1</sub>
&nbsp;&nbsp;&nbsp;&nbsp;    **for** _l_ = 1 **to** _n_  
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;       **for** _i_ = 1 **to** _n_ - _l_ + 1  
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;           _j_ = _i_ + _l_ - 1  
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;           _e_\[_i_, _j_] = ∞  
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;           _w_\[_i_, _j_] = _w_\[_i_, _j_ - 1] + _p_<sub>j</sub> + _q_<sub>j</sub>  
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;           **for** _r_ = _i_ **to** _j_  
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;               _t_ = _e_\[_i_, _r_ - 1] + _e_\[_r_ + 1, _j_] + _w_\[_i_, _j_]  
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;               **if** _t_ < _e_\[_i_, _j_]  
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;                   _e_\[_i_, _j_] = _t_  
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;                   _root_\[_i_, _j_] = _r_  
&nbsp;&nbsp;&nbsp;&nbsp;    **return** _e_ and _root_

**In Kotlin**: [Implementation](../src/main/kotlin/chapter15/OptimalBinarySearchTree.kt) and [Tests](../src/test/kotlin/chapter15/OptimalBinarySearchTreeTest.kt).
